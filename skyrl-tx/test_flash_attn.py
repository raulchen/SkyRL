#!/usr/bin/env python3
"""Simple test client for TX server with flash attention using tinker API."""
...
def test_sample(sampling_client, tokenizer, num_samples: int = 4, max_tokens: int = 32):
    ...
    print(f"\n=== Testing Sampling (n={num_samples}, max_tokens={max_tokens}) ===")
    ...
    request = sampling_client.sample(
        prompt=prompt,
        sampling_params=types.SamplingParams(temperature=0.7, max_tokens=max_tokens, seed=42),
